...
class PageSegementView :UIView {
    ...
    lazy var pageDataSource = DataSourceCollectionView<PageViewCell,Category>(collection: pageCollection, cellSize: { index in
    ...
    override init(frame: CGRect) {
        ...
        self.addActivityView(activityLoader: activity)
        getCategory()
    }
    ...
    fileprivate func getCategory() {
        DataSourceProvider.stander.getCategory { categoriesList, error in
            if error != nil {
                return
            }
